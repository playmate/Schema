...
with st.expander("👤 Personal", expanded=False):
    ...
    for n in st.session_state.people:
        ...
        # --- Vertikal layout för arbetstider ---
        with st.expander(f"Arbetstider för {n}", expanded=False):
            for dag in veckodagar:
                ...
                if tillgang:
                    ...
                    col_time = st.columns([1,1])
                    with col_time[0]:
                        start_time = st.time_input(f"Start {dag}", value=start_prev, key=f"start_{n}_{dag}", step=900)
                    with col_time[1]:
                        end_time = st.time_input(f"Slut {dag}", value=end_prev, key=f"end_{n}_{dag}", step=900)
